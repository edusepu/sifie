...
$consulta = "SELECT * FROM efectos LEFT JOIN detallecomputadora ON efectos.id=detallecomputadora.idDetalle LEFT JOIN monitores ON monitores.id=detallecomputadora.monitor WHERE detallecomputadora.monitor=" . $id." and efectos.activo=0";
...
foreach ($data as $datos) {
    ...
    $idEfecto=$datos['idDetalle'];
    $existe=1;
    $monitor = $datos['marca']." ".$datos['modelo']." ".$datos['tamanio']."''";
}
$consulta = "SELECT * FROM fundacion LEFT JOIN detalleFundacion ON fundacion.id=detalleFundacion.idDetalle LEFT JOIN monitores ON monitores.id=detalleFundacion.monitor WHERE detalleFundacion.monitor=".$id." and fundacion.activo=0";
$resultado = $conexion->prepare($consulta);
$resultado->execute();
$data = $resultado->fetchAll(PDO::FETCH_ASSOC);
foreach ($data as $datos) {
            ...
            <div class="row">
                ...
                <div class="col-md-8">
                    <div style="text-align: center">
                        ...
                        <h3>Detalle Efecto <?php echo "$idEfecto";?> </h3>
                        <br>
                    </div>
                        <table class="table">
                        <thead>
