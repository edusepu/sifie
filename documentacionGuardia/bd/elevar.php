...
switch ($opcion) {
    case 1: //Elevar al J GU
        //$estado = $estado+1;
        $consulta = "UPDATE registroplanillas SET estado=2 WHERE idPlanilla='$idPlanilla' and fecha='$fechaElevar' ";
        $resultado = $conexion->prepare($consulta);
        $resultado->execute();
        $data = $resultado->fetchAll(PDO::FETCH_ASSOC);
        break;
    case 2: //Elevar al OF Ser
        $consulta = "UPDATE registroplanillas SET estado=3 WHERE idPlanilla='$idPlanilla' and fecha='$fechaElevar' ";
        $resultado = $conexion->prepare($consulta);
        $resultado->execute();
        $data = $resultado->fetchAll(PDO::FETCH_ASSOC);
        break;
...
//        break;
    case 3: //Elevar al J Dpto Apoyo
        $consulta = "UPDATE registroplanillas SET estado=4 WHERE idPlanilla='$idPlanilla' and fecha='$fechaElevar' ";
        $resultado = $conexion->prepare($consulta);
        $resultado->execute();
        $data = $resultado->fetchAll(PDO::FETCH_ASSOC);
        break;
